...
export const ThumbStick: FC<ThumbStickProps> = ({
  ...
  const updateArea = useCallback((): void => {
    ...
    const { left: observerX, top: observerY } =
  ...
  const moveStick = useCallback(
    (event: React.PointerEvent | PointerEvent): void => {
      ...
      if (magnitude > areaRadius) {
        ...
        if (sticky) {
          translateStick(
            event.clientX - direction.x,
            event.clientY - direction.y,
          );
          setAreaCenter(
            event.clientX - direction.x,
            event.clientY - direction.y,
  ...
  return (
    ...
    >
      <div ref={areaRef} className={`thumb-stick thumb-stick_${align}`}>
        <div ref={controlRef} className="thumb-stick__control" />
      </div>
    </div>
  );
};
